...
TEST_SUITE("Manager")
{
    ...
    TEST_CASE ("attach_wire_to_connector(): Attaching a wire to a connector")
    {
        ...
        SUBCASE("The wires is on the connector") {
            ...
            REQUIRE_EQ(manager.attached_wire(&conn)->wire, wire.get());
            ...
            REQUIRE_EQ(manager.attached_wire(&conn), std::nullopt);
            REQUIRE_EQ(manager.point_is_attached(wire.get(), 0), false);
    ...
    TEST_CASE("Connections are updated when a points is inserted or removed")
    {
        ...
        REQUIRE_EQ(manager.attached_wire(&conn1)->point_index, 0);
        REQUIRE_EQ(manager.attached_wire(&conn2)->point_index, 1);
        ...
        // Make sure the correct points are attached to the connectors
        REQUIRE_EQ(manager.attached_wire(&conn1)->point_index, 0);
        REQUIRE_EQ(manager.attached_wire(&conn2)->point_index, 2);
        ...
        // Make sure the correct points are attached to the connectors
        REQUIRE_EQ(manager.attached_wire(&conn1)->point_index, 0);
        REQUIRE_EQ(manager.attached_wire(&conn2)->point_index, 3);
        ...
        // Make sure the correct points are attached to the connectors
        REQUIRE_EQ(manager.attached_wire(&conn1)->point_index, 0);
        REQUIRE_EQ(manager.attached_wire(&conn2)->point_index, 4);
