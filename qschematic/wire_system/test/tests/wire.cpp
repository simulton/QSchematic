...
TEST_SUITE("Wire")
{
    ...
    TEST_CASE("Add and remove points")
    {
        ...
        SUBCASE("Point can be added")
        {
            ...
            REQUIRE_EQ(wire->points_count(), 3);
            REQUIRE_EQ(wire->points().at(0), QPointF(0, 10));
            REQUIRE_EQ(wire->points().at(1), QPointF(23.2, 100));
            REQUIRE_EQ(wire->points().at(2), QPointF(-123.4, 0.23));
        }
        ...
        SUBCASE("Points can be removed")
        {
            ...
            REQUIRE_EQ(wire->points().at(0), QPointF(0, 10));
            REQUIRE_EQ(wire->points().at(1), QPointF(-123.4, 0.23));
        }
    }
    ...
    TEST_CASE("Wires can be moved")
    {
        ...
        CHECK_EQ(wire1->points().at(0).toPointF(), QPointF(1, 9));
        CHECK_EQ(wire1->points().at(1).toPointF(), QPointF(6, 9));
        CHECK_EQ(wire1->points().at(2).toPointF(), QPointF(6, 4));
        CHECK_EQ(wire1->points().at(3).toPointF(), QPointF(16, 4));

        // Make sure the second wire stayed on the first one
        CHECK_EQ(wire2->points().at(0).toPointF(), QPointF(11, 4));
        CHECK_EQ(wire2->points().at(1).toPointF(), QPointF(11, 10));
        CHECK_EQ(wire2->points().at(2).toPointF(), QPointF(15, 10));
        ...
        CHECK_EQ(wire2->points().at(0).toPointF(), QPointF(11, 4));
        CHECK_EQ(wire2->points().at(1).toPointF(), QPointF(11, 11));
        CHECK_EQ(wire2->points().at(2).toPointF(), QPointF(17, 11));
    }
    ...
    TEST_CASE("point_is_on_wire()")
    {
        ...
        CHECK(wire->point_is_on_wire(QPointF(10, 20)));
        CHECK(wire->point_is_on_wire(QPointF(10, 10)));
        CHECK(wire->point_is_on_wire(QPointF(15, 10)));
        CHECK(wire->point_is_on_wire(QPointF(12.34, 10.01)));
        CHECK(wire->point_is_on_wire(QPointF(20.01, 10)));
        CHECK(wire->point_is_on_wire(QPointF(6, 8)));
        CHECK(wire->point_is_on_wire(QPointF(8.2345, 3.531)));

        // These points are not on the wire
        CHECK_FALSE(wire->point_is_on_wire(QPointF(-10, -100)));
        CHECK_FALSE(wire->point_is_on_wire(QPointF(5, 5)));
        CHECK_FALSE(wire->point_is_on_wire(QPointF(6, 11)));
        CHECK_FALSE(wire->point_is_on_wire(QPointF(15, 15)));
        CHECK_FALSE(wire->point_is_on_wire(QPointF(15, 11)));
        CHECK_FALSE(wire->point_is_on_wire(QPointF(15, 10.1)));
        CHECK_FALSE(wire->point_is_on_wire(QPointF(15, 9.9)));
    }
    ...
    TEST_CASE("move_point_by(): Move points while making sure the angles are preserved")
    {
        ...
        SUBCASE("Move the first point")
        {
            ...
            CHECK_EQ(wire->points().at(0).toPointF(), QPointF(0, 60));
            CHECK_EQ(wire->points().at(1).toPointF(), QPointF(40, 60));

            // Make sure the other points didn't move
            CHECK_EQ(wire->points().at(2).toPointF(), QPointF(40, 0));
            CHECK_EQ(wire->points().at(3).toPointF(), QPointF(20, 0));
            CHECK_EQ(wire->points().at(4).toPointF(), QPointF(20, 40));
            CHECK_EQ(wire->points().at(5).toPointF(), QPointF(80, 40));
        }
        ...
        SUBCASE("Move a point in the middle of the wire")
        {
            ...
            CHECK_EQ(wire->points().at(0).toPointF(), QPointF(0, 80));
            CHECK_EQ(wire->points().at(1).toPointF(), QPointF(40, 80));
            CHECK_EQ(wire->points().at(2).toPointF(), QPointF(40, 0));

            // Make the point and its adjacent points have moved
            CHECK_EQ(wire->points().at(3).toPointF(), QPointF(0, 0));
            CHECK_EQ(wire->points().at(4).toPointF(), QPointF(0, 60));
            CHECK_EQ(wire->points().at(5).toPointF(), QPointF(80, 60));
        }
    }
}